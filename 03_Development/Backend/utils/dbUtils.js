...
CREATE TABLE IF NOT EXISTS appointments(
  ...
  INDEX idx_exp_status (expires_at, status)
)`;
...
CREATE TABLE IF NOT EXISTS token_logs (
    ...
    qr_token VARCHAR(512) NOT NULL COMMENT 'Reference to appointments.qr_token',
    scanned_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_valid BOOLEAN DEFAULT true,
    verification_result ENUM('valid', 'expired', 'invalid', 'used') NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
const createTable = async (tableName, query) => {
    try {
        ...
        const updatePathsQuery = `
      UPDATE doctors
      SET 
        certification_path = SUBSTRING_INDEX(certification_path, '/', -1),
        id_proof_path = SUBSTRING_INDEX(id_proof_path, '/', -1)
      WHERE 
        certification_path LIKE '%/kyc/%' 
        OR id_proof_path LIKE '%/kyc/%';
    `;
    const [updateResult] = await pool.query(updatePathsQuery);
    console.log(`Updated ${updateResult.affectedRows} doctor records to store filenames.`);
