...
export const registerDoctor = async (req, res) => {
    ...
    try {
        ...
        if (!req.files || !req.files['certification'] || !req.files['id_proof']) {
            return res.status(400).json({ success: false, message: "Certification and ID proof files are required." });
        }
        ...
        const certificationPath = path.join('uploads', certificationFile.filename).replace(/\\/g, "/");
        const idProofPath = path.join('uploads', idProofFile.filename).replace(/\\/g, "/");

        connection = await pool.getConnection();
        await connection.beginTransaction();

        // Register user with transaction connection
        ...
        // Register doctor
        const doctorResponse = await registerDoctorInService({
            ...req.body,
        ...
        if (doctorResponse.success) {
            ...
            await connection.rollback();
            return res.status(400).json(doctorResponse);
        }
    } catch (error) {
        if (connection) {
            await connection.rollback();
